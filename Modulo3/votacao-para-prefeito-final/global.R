...
library(summaryBox)
# ls("package:summaryBox")
# browseURL("https://www.listendata.com/2021/03/value-info-box-in-shiny-rmarkdown.html")
...
##' @title Filtra os dados conforme escolhas de ano e UF
##'
##' @param tb \code{data.frame} A tabela de dados com informações das
##'     votações para prefeito.
##' @param br_municipal \code{data.frame} A tabela de dados com
##'     informações dos polígonos para os municípios brasileiros.
##' @param unidade_federativa \code{chr[1]} A sigla de uma unidade
##'     federativa.
##' @param ano_eleitoral \code{int[1]} O ano eleitoral.
##' @return \code{list} Uma lista com 2 elementos.
...
##' @title Constroí um gráfico de barras para frequência de partidos
##'
##' @param tb_count \code{data.frame} Uma tabela com a frequência
##'     absoluta e relativa dos partidos em cada UF.
##' @inheritParams filter_data
##' @return Retorna um gráfico de barras feito com \link{ggplot2}.
##' @author Walmes Zeviani
##' @examples
##'
...
##' @title Constroí um gráfico de barras para frequência de partidos
##'
##' @param tb_estado \code{data.frame} Uma tabela com polígonos e
##'     partido que ganhou em cada para município.
##' @inheritParams filter_data
##' @return Retorna um mapa temático feito com \link{ggplot2}.
##' @author Walmes Zeviani
##' @examples
##'
##' tb_test <- filter_data(tb, br_municipal, "RJ", 2000)
##' make_cloropleth(tb_test$tb_estado, "RJ", 2000)
##' rm(tb_test)
##'
...
##' @title Faz tabela usando o {reactbale}
##'
##' @inheritParams make_barplot
##' @inheritParams make_cloropleth
##' @return Retorna uma tabela para HTML feita com \link{reactable}.
##' @author Walmes Zeviani
##' @examples
##'
##' tb_test <- filter_data(tb, br_municipal, "MS", 2000)
